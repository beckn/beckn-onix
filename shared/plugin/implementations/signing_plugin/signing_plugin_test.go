...
func TestParseConfig_Success(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, int64(3600), parsedConfig.TTL)
}

func TestParseConfig_MissingTTL(t *testing.T) {
...
func TestSignerProvider_New_Success(t *testing.T) {
	provider := SignerProvider{}
	config := map[string]string{"ttl": "3600"}

	signer, err := provider.New(context.Background(), config)
	assert.NoError(t, err)
	assert.NotNil(t, signer)
}

func TestSignerProvider_New_InvalidConfig(t *testing.T) {
	provider := SignerProvider{}
	config := map[string]string{"ttl": "invalid"}

	_, err := provider.New(context.Background(), config)
	assert.Error(t, err)
}
