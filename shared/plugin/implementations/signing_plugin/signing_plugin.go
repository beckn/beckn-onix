...
func parseConfig(config map[string]string) (SigningConfig, error) {
	ttlStr, exists := config["ttl"]
	if !exists {
		return SigningConfig{}, errors.New("ttl not found in config")
	}

	ttl, err := strconv.ParseInt(ttlStr, 10, 64)
	if err != nil {
		return SigningConfig{}, fmt.Errorf("invalid ttl value: %w", err)
	}

	return SigningConfig{TTL: ttl}, nil
}
