...
func Decode(encoding string, b []byte, maxDecompressed int64) ([]byte, error) {
	...
	if !ok {
		return nil, catalog.Permanentf("catalogcrawler: unsupported encoding %q", encoding)
	}
	rc, err := d(b)
	if err != nil {
		return nil, err
	...
	if err != nil {
		return nil, catalog.Permanentf("catalogcrawler: decode %q: %v", encoding, err)
	}
	if int64(len(out)) > maxDecompressed {
		return nil, catalog.Permanentf("catalogcrawler: decoded body exceeds max %d bytes", maxDecompressed)
	}
	return out, nil
}
