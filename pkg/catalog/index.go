...
// entry's self-signature. This is the RFC-mandated document shape both a
// crawler (reading) and a publisher/store (writing) share -- ported from
// pkg/catalog/crawler's own index.go so both sides of the chain encode it
// once, not twice.
...
// discovery, never trusted on the strength of this hint alone. Version is
// the MASTER's baseline.version last validated against -- a publisher's
// caller keeps it current as that changes; a reader that doesn't act on
// dependency versions yet (e.g. today's crawler) is free to ignore it.
