...
func TestCheckCompatibility_UnknownType(t *testing.T) {
	...
	// Types absent from the local manifest have no translation target; they are skipped.
	if len(needs) != 0 {
...
func TestCheckCompatibility_MixedOutcomes(t *testing.T) {
	...
	// Only Item (version mismatch) should be in needs; Quote (unknown type) is skipped.
	if len(needs) != 1 {
	...
	if findNeedByType(needs, "Quote") != nil {
		t.Error("Quote is unknown type — should be skipped, not in needs")
	}
}
...
func TestCheckCompatibility_EmptyManifest(t *testing.T) {
	...
	m := manifest() // no schema objects — unknown types are skipped
	...
	if len(needs) != 0 {
		t.Fatalf("expected 0 translations needed for unknown type, got %d", len(needs))
	}
}
