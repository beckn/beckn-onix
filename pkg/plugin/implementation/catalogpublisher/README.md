...
- **Content-based `catalogType` inference when `publishDirectives` is
  ...
  offers present -> `REGULAR`, resources-only -> `MASTER`. This handler
...
this handler at publish time). `catalogType` (`MASTER`/`REGULAR`) is
...
`publishDirectives[].schemaTypes` is this handler's own extension of the
...
`validatePublishRequest` (`catalogPublishHandler.go`) checks referential
and business-rule constraints a JSON Schema can't express -- one array's
values being drawn from another array in the same document isn't
something JSON Schema can assert -- so these run unconditionally, even
with `schemaValidator` unconfigured, and reject the whole request with
`400` before `Publish` is ever called (collecting every violation found,
not just the first):
...
- `publishDirectives[].resourceDirectives[].resourceId` must match a
  resource `id` actually present in that catalog.
- `publishDirectives[].updateMode`: `"MERGE"` (or omitted) is accepted;
  ...
  handler doesn't do yet); any other value is rejected too.
- `publishDirectives[].schemaTypes` is checked against a custom JSON
  Schema (`catalogpublish_schematypes.go`) derived from NFH-014's own
  documented shape (a unique, non-empty array of domain-schema context
  URIs) -- there's no upstream machine-readable schema for it to reuse
  (see the note above), and `schemaValidator`'s own auxiliary-spec
  mechanism can't extend `catalog/publish`'s schema to add it either
  (auxiliary specs may only add actions beckn.yaml doesn't already
  define, never extend one it does).

Per-catalog *content* issues (missing/empty `id`, bad `descriptor`) are
deliberately left to `catalogpublisher`'s own validate stage instead,
which reports them as a non-fatal `PublishError` for that one catalog
rather than failing the whole request -- see
`TestCatalogPublishHandler_InvalidCatalogIsRejectedNotFatal`.
