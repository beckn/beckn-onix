...
// It resolves a networkId to the index URLs of the provider nodes that
// publish a catalog. Framework-agnostic (net/http + encoding/json only).
//
// Copied over from the catalog-crawler prototype as-is: the dediregistry
// plugin doesn't support the /query API yet (a separate piece of work), so
// this is a standalone client for now rather than a reuse of that plugin.
//
// The {base} registry URL is TRUSTED deployment config, so pkg/crawler's SSRF
// guard is deliberately NOT applied to this GET -- that guard exists for the
// UNtrusted publisher artifact URLs this lookup discovers, and it still runs
// on them later via catalog.Fetcher. Nothing here weakens that.
...
// maxQueryBytes caps the /query response read into memory, so a runaway
// registry cannot exhaust the crawler. The response is metadata (one record
// per network participant), so a generous few MiB is far more than any real
// network needs.
...
// NewDediQueryClient builds a query client against the DeDi registry rooted
// at baseURL (e.g. https://fabric.nfh.global/registry/dedi).
...
// queryResponse is the subset of the /query envelope this client consumes.
...
// queryMeta is the registry's generic, schema-agnostic meta object
// (CON-TBD-33: "a PN MUST place catalog_index_urls in its Beckn Subscriber
// record's meta object, and a DS MUST look for it there"). It is a LIST of
// {url} objects, not a single URL string -- a node may host more than one
// catalog index (e.g. separating a fast-moving retail catalog from a
...
// record serializing it twice) instead of a native array.
// parseCatalogIndexURLs tolerates both shapes.
...
// catalog per node). networkID is used verbatim as the
...
func (c *DediQueryClient) Providers(ctx context.Context, networkID string) ([]Provider, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("catalogcrawler: registry query %s: %w", url, err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("catalogcrawler: registry query %s: status %d", url, resp.StatusCode)
	}
	b, err := io.ReadAll(io.LimitReader(resp.Body, maxQueryBytes+1))
	if err != nil {
		return nil, fmt.Errorf("catalogcrawler: reading registry query %s: %w", url, err)
	}
	if int64(len(b)) > maxQueryBytes {
		return nil, fmt.Errorf("catalogcrawler: registry query %s exceeds max %d bytes", url, maxQueryBytes)
	}

	var qr queryResponse
	if err := json.Unmarshal(b, &qr); err != nil {
		return nil, fmt.Errorf("catalogcrawler: parsing registry query %s: %w", url, err)
	}
...
// deadline; a non-positive timeout means no limit. It mirrors catalog.Fetcher's
// own budgeting so a registry that never answers cannot stall a pass.
