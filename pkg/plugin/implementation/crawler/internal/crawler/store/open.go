// Package store is the crawler's storage layer. It owns the pluggable-backend
// contract (Backend and the backend-agnostic config in provider.go) and ships
// Postgres as the bundled backend, selected by name through the factory:
// embedded idempotent migrations for the three tables (crawler_index,
// crawler_queue, crawler_catalog) plus a thin Store over database/sql (pgx
// driver). It is ONE Go package so the catalog cursor and the queue can share a
