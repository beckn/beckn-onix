...
func TestPassReportHistory(t *testing.T) {
	...
	for i := 1; i <= passHistoryCap+5; i++ {
		must(t, s.UpsertCatalog(ctx, catalog.CatalogState{
			...
			Report: catalog.PassReport{ToVersion: int64(i), Mode: "FULL", Resources: i, Outcome: "pushed"},
		}))
	}
...
func TestPassReportRecordsPartial(t *testing.T) {
	...
	must(t, s.RecordFailure(ctx, cid, "i", "", catalog.PassReport{
...
func TestCatalogCursor(t *testing.T) {
	...
	if err := s.UpsertCatalog(ctx, catalog.CatalogState{
		CatalogID: "p/c", IndexURL: "https://x/index.json", ParticipantID: "p",
		Version: 42, Status: "active", Report: catalog.PassReport{Outcome: "pushed"},
	}); err != nil {
		t.Fatal(err)
	}
