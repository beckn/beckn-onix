...
// every non-routable range it misses is a host that would actually get dialed.
...
// fault, so a caller parks/rejects it rather than retrying an internal address
// forever.
func TestCheckPublicURL_NonRoutableRanges(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assertPermanentFault(t, checkPublicURL(context.Background(), tt.url), FaultSSRF)
		})
	}
}
