...
// CGNAT, unspecified, 0.0.0.0/8, 240.0.0.0/4), so a fetched URL can't be
...
// loopback/private/link-local addresses (refuses private-address URLs for
// untrusted fetched content).
//
// ctx bounds the DNS lookup: the host comes from untrusted content, so a
// resolver that is merely slow -- or one that deliberately never answers --
// must not stall the check past the caller's deadline.
func checkPublicURL(ctx context.Context, raw string) error {
	...
	for _, ip := range ips {
		if !isPublicIP(ip) {
			// Permanent: the URL points at an internal address, which no amount of
			// retrying fixes.
			return PermanentFaultf(FaultSSRF, "crawler: refusing private/loopback host %q", host)
		}
	}
	return nil
}
	...
	return func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		for _, ipa := range ips {
			if !isPublicIP(ipa.IP) {
				...
				lastErr = PermanentFaultf(FaultSSRF, "crawler: refusing private/loopback address %s for %q", ipa.IP, host)
				continue
			}
