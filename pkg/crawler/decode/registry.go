// Package decode turns already-digest-verified artifact bytes into decoded
// content. It is the format extension point: adding zstd/brotli is one entry
// in the registry + one decoder func, with fetch/verify/parse untouched. The
...
// codecs maps an encoding name to its decoder.
...
func Decode(encoding string, b []byte, maxDecompressed int64) ([]byte, error) {
	...
	if !ok {
		return nil, crawler.Permanentf("crawler: unsupported encoding %q", encoding)
	}
	rc, err := d(b)
	if err != nil {
		return nil, err
	...
	if err != nil {
		return nil, crawler.Permanentf("crawler: decode %q: %v", encoding, err)
	}
	// Stays unclassified (=> FaultDecode, "couldn't unpack the content"): a
	// decompression bomb is caught while unpacking, so that IS the truthful
	// stage. FaultOversize is reserved for the download cap in crawler.Client,
	// whose phrase is "download the artifact". Both are permanent; only the
	// stage differs.
	if int64(len(out)) > maxDecompressed {
		return nil, crawler.Permanentf("crawler: decoded body exceeds max %d bytes", maxDecompressed)
	}
	return out, nil
}
...
// EncodingFor picks an artifact's encoding: the explicit entryEncoding if
// set, else inferred from the URL suffix (.json.gzip / .json.gz => gzip).
