...
func TestDecode_CorruptGzipPermanent(t *testing.T) {
	if _, err := Decode("gzip", []byte("this is not gzip"), 1<<20); err == nil || !crawler.IsPermanent(err) {
		t.Fatalf("corrupt gzip must be a permanent error, got %v", err)
	}
}

func TestDecode_UnknownEncodingPermanent(t *testing.T) {
	if _, err := Decode("zstd", []byte("x"), 1<<20); err == nil || !crawler.IsPermanent(err) {
...
// no change to the client/verify/decode call sites.
